    ...
    return new Promise((resolve) => {
        try {
            ...
            jwt.verify(token, secret, async (er, decode) => {
                if (er) {
                    return resolve({ message: "session expired login again.", success: false });
                }

                if (decode) {
                    // console.log("decode >>>> ", decode);
                    return resolve({ message: "valid token", success: true, decoded: decode });
                }
            ...
            return resolve({ message: "session expired.", success: false, error: error.message });
        }
    });
}
